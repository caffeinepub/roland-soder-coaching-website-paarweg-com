...
function buildDistinguishedName(metadata: CSRMetadata): Uint8Array {
  ...
  if (metadata.country) {
    components.push(
      buildRDN([0x06, 0x03, 0x55, 0x04, 0x06], metadata.country, 0x13),
    ); // PrintableString
  }
  ...
  if (metadata.state) {
    components.push(
      buildRDN([0x06, 0x03, 0x55, 0x04, 0x08], metadata.state, 0x0c),
    ); // UTF8String
  }
  ...
  if (metadata.city) {
    components.push(
      buildRDN([0x06, 0x03, 0x55, 0x04, 0x07], metadata.city, 0x0c),
    ); // UTF8String
  }
  ...
  if (metadata.company) {
    components.push(
      buildRDN([0x06, 0x03, 0x55, 0x04, 0x0a], metadata.company, 0x0c),
    ); // UTF8String
  }
  ...
  if (metadata.commonName) {
    components.push(
      buildRDN([0x06, 0x03, 0x55, 0x04, 0x03], metadata.commonName, 0x0c),
    ); // UTF8String
  }
...
function buildRDN(
  ...
  const valueWithType = new Uint8Array([
    stringType,
    valueBytes.length,
    ...valueBytes,
  ]);
